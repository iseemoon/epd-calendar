...
#include "include/epd_panel_io.h"
...
static const char *TAG = "epd_panel.io.spi";

static esp_err_t panel_io_spi_rx_param(epd_panel_io_t *io, int lcd_cmd, void *param, size_t param_size);
static esp_err_t panel_io_spi_tx_param(epd_panel_io_t *io, int lcd_cmd, const void *param, size_t param_size);
static esp_err_t panel_io_spi_tx_color(epd_panel_io_t *io, int lcd_cmd, const void *color, size_t color_size);
static esp_err_t panel_io_spi_del(epd_panel_io_t *io);
static void lcd_spi_pre_trans_cb(spi_transaction_t *trans);
static void lcd_spi_post_trans_color_cb(spi_transaction_t *trans);
static esp_err_t panel_io_spi_register_event_callbacks(epd_panel_io_handle_t io, const epd_panel_io_callbacks_t *cbs, void *user_ctx);
...
typedef struct {
    epd_panel_io_t base;         // Base class of generic lcd panel io
    spi_device_handle_t spi_dev; // SPI device handle
    int dc_gpio_num;             // D/C line GPIO number
    epd_panel_io_color_trans_done_cb_t on_color_trans_done; // User register's callback, invoked when color data trans done
...
esp_err_t epd_new_panel_io_spi(spi_host_device_t bus, const epd_panel_io_spi_config_t *io_config, epd_panel_io_handle_t *ret_io)
{
    ...
    epd_panel_io_spi_t *spi_panel_io = NULL;
    ESP_GOTO_ON_FALSE(bus && io_config && ret_io, ESP_ERR_INVALID_ARG, err, TAG, "invalid argument");
    spi_panel_io = calloc(1, sizeof(epd_panel_io_spi_t) + sizeof(lcd_spi_trans_descriptor_t) * io_config->trans_queue_depth);
    ...
    spi_panel_io->lcd_cmd_bits = io_config->epd_cmd_bits;
    spi_panel_io->lcd_param_bits = io_config->epd_param_bits;
...
static esp_err_t panel_io_spi_del(epd_panel_io_t *io)
{
    ...
    epd_panel_io_spi_t *spi_panel_io = __containerof(io, epd_panel_io_spi_t, base);
...
static void spi_lcd_prepare_cmd_buffer(epd_panel_io_spi_t *panel_io, const void *cmd)
{
...
static void spi_lcd_prepare_param_buffer(epd_panel_io_spi_t *panel_io, const void *param, size_t param_size)
{
...
static void lcd_spi_pre_trans_cb(spi_transaction_t *trans)
{
    epd_panel_io_spi_t *spi_panel_io = trans->user;
